...
public class DigiAccountsOverview_Postpaid_Util extends PageNavigation {
	
	protected  Logger log = Logger.getLogger(DigiAccountsOverview_Postpaid_Util.class);
	...
	protected  void setUp(){
		...
		signInWithMSSIDN_stage(PageNavigation.prop.getProperty(AppConstants.COUNTRY_PREFIX),
						 PageNavigation.prop.getProperty(AppConstants.AccountsOverview_POSTPAID_MSISDN),
						 PageNavigation.prop.getProperty(AppConstants.AccountsOverview_POSTPAID_PASS));
		movetoAccountsOverviewPostpaid();
		readSourceDataFiles();
		/*waitgetForPageLoad(200) ;
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@id='menu--subnav']/div/div[2]/ul/li[5]/a")));*/
		/*}catch(Exception e){
			System.out.println("Error");
		}*/
		}

	protected void movetoAccountsOverviewPostpaid()
	{
	    waitForPageLoad(60);
	    handleLink_ByText("MyDigi");
	    waitForPageLoad(30);
		//handleLink_ByText("Accounts & Plans");
		//*[@id="content"]/div[13]/div/div/div[2]/button[1]
	...
	/**
     ...
     private void readSourceDataFiles(){
    	String statusFileName = PageNavigation.prop.getProperty(AppConstants.CONFIG_FILES_LOCATION)
    											   .concat(AppConstants.DIGIACCOUNTSOVERVIEW_POSTPAID_DATASTATUSFILE_LOCATION);
    	...
    	String dataFileName = PageNavigation.prop.getProperty(AppConstants.CONFIG_FILES_LOCATION)
												 .concat(AppConstants.DIGIACCOUNTSOVERVIEW_POSTPAID_STATICDATAFILE_LOCATION);
    	 ...
    	 if(status.equalsIgnoreCase(AppConstants.YES)){
    		 ...
    		 try {
    			...
    			String msisdnNo =  prop.getProperty(AppConstants.COUNTRY_PREFIX).concat(
    							   prop.getProperty(AppConstants.AccountsOverview_POSTPAID_MSISDN)
    								);
