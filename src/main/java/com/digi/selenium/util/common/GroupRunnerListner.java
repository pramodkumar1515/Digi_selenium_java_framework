...
public class GroupRunnerListner extends PageNavigation  implements IMethodInterceptor {
	...
	public List intercept(java.util.List<IMethodInstance> methods,	ITestContext context) {
		...
		for (IMethodInstance m : methods) {
			...
			if (groups.contains(getSourceData("GROUP1"))||groups.contains(getSourceData("GROUP2"))||groups.contains(getSourceData("GROUP3"))) {
				result.add(m);
			} else {
